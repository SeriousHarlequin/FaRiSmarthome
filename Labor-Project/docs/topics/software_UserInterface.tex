...
\end{lstlisting}\end{minipage}
    ...
    Anstatt des Touchscreens wurde ein Rotary Encoder verwendet, um die
    Benutzeroberfläche zu steuern. Da \texttt{lvgl} nur eingeschränkte
    Unterstützung für Rotary Encoder bietet, wurde eine eigene Implementierung
    geschrieben. 

        \subsubsection{lv\_groups}
        lv\_groups sind Gruppen von Objekten, durch die iteriert werden kann.
        Dies Eigenschaft wird verwendet, um die Objekte in der Benutzeroberfläche
        mit dem Rotary Encoder auszuwählen. \\~\\

        Vereinfacht kann die Funktionsweise wie folgt dargestellt werden:
        \begin{lstlisting}[style=cppCode]
    ...
    if(encoder.getCount() < -1){
      ...
      focusPrev(currentGroup);
        ...
        Die momentane Gruppe ändert sich, wenn auf eine andere Ansicht 
        gewechselt wird. So kann der Code allgemein gehalten werden und
        muss nicht für jede Ansicht angepasst werden. 
